...
async function testComfyuiProvider(baseUrl: string, apiKey?: string): Promise<TestProviderResult> {
  ...
  try {
    ...
    const device = data.devices?.[0]?.name || 'unknown'
    steps.push({
      name: 'models',
      status: 'pass',
      message: `ComfyUI v${version} connected (${device})`,
    })
    return { success: true, steps }
  } catch (error) {
    steps.push({
...
export async function testProviderConnection(payload: TestProviderPayload): Promise<TestProviderResult> {
  ...
  // ComfyUI doesn't require apiKey
  if (apiType !== 'comfyui' && !apiKey) {
    return {
      success: false,
  ...
  // ComfyUI doesn't require apiKey
  if (apiType === 'comfyui' && !apiKey) {
    // Allow empty apiKey for ComfyUI (local server)
  }
