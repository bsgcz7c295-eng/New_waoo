...
type CustomProviderType = 'gemini-compatible' | 'openai-compatible' | 'comfyui'
...
export function ApiConfigTabContainer() {
  ...
  const doAddProvider = useCallback(() => {
    ...
    let apiMode: 'openai-official' | 'gemini-sdk' | undefined

    if (newGeminiProvider.apiType === 'comfyui') {
      providerId = 'comfyui'
      apiMode = undefined
    } else {
      providerId = `${newGeminiProvider.apiType}:${uuid}`
    ...
    let apiKey = newGeminiProvider.apiKey.trim()

    if (newGeminiProvider.apiType === 'comfyui' && !apiKey) {
  ...
  const handleAddGeminiProvider = useCallback(async () => {
    if (!newGeminiProvider.name) {
      alert(tp('fillRequired'))
      return
    }
    if (newGeminiProvider.apiType !== 'comfyui' && !newGeminiProvider.baseUrl) {
      alert(tp('fillRequired'))
      return
    }
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="space-y-6 p-6">
          ...
          {modelProviders.some((p) => getProviderKey(p.id) === 'comfyui') && (
            <div className="glass-surface rounded-2xl border border-[var(--glass-stroke-base)] p-5">
              <WorkflowManager />
            </div>
          )}
        </div>
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <div className="relative">
              <select
                ...
                onChange={(event) => {
                  const newType = event.target.value as CustomProviderType
                  setNewGeminiProvider({
                    ...newGeminiProvider,
                    apiType: newType,
                    baseUrl: newType === 'comfyui' ? 'http://127.0.0.1:8188' : newGeminiProvider.baseUrl,
                    name: newType === 'comfyui' && !newGeminiProvider.name ? 'ComfyUI Local' : newGeminiProvider.name,
                  })
                }}
                disabled={testStatus === 'testing'}
          ...
          <div>
            ...
            <input
              ...
              disabled={testStatus === 'testing'}
              placeholder={newGeminiProvider.apiType === 'comfyui' ? 'http://127.0.0.1:8188' : t('baseUrl')}
